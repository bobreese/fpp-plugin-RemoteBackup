...
- **Documented:** reworded finding #1's system-config sentence in
  ...
  limitation showed up as an afterthought at the end, and understated it besides (it's
  manually restorable, just not through any guided flow, not unrestorable outright). Now
  leads with what actually matters - getting it back onto a system means untarring the
  archive and copying files into place by hand, no guided restore from FPP or this plugin.
