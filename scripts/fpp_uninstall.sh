...
#
# This used to also check the old flat-file media-root location and
# clean it up here too, for an install that hadn't yet run a script since
# the plugindata/ move. Retired along with that move's own migration
# code (see ajax.php's rb_settings_external_backup_path()) - long enough
# in the field that this is no longer a real scenario.
...
if [ "$PURGE_BACKUPS" = "1" ] && [ "${#BACKUP_DIRS[@]}" -gt 0 ]; then
    ...
    for d in "${BACKUP_DIRS[@]}"; do
        ...
        # remote's media tree can contain root-owned content (that
