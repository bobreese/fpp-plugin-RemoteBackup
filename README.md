...
- **What it changes:** on your selection of a remote to back up, pushes its own SSH key to that remote's `authorized_keys` for passwordless access — the config page discloses this at the point of selection, and it's removed again, best-effort, on uninstall; can mount a USB drive via `/etc/fstab`; installs a handful of packages (`rsync`, `jq`, `sshpass`, etc.) via `apt-get` if missing; sets FPP's restart flag after install/uninstall.
